...
public class MissionOfferAssembler {
    ...
    public MissionOfferResponse getMissionOffer(ApiUser apiUser, CreateMissionOfferRequest request) {
        MissionOfferCreateResult createResult = missionOfferService.getOrCreateOffer(apiUser, request.matchId());
        MatchResult matchResult = matchService.findMatchResult(apiUser, request.matchId());
